...
【版面結構】
・主商品：置於畫面右側，佔約 75%，是畫面「唯一」絕對主角；完整呈現、不裁切、不變形、高解析。要有真實商品攝影棚質感——底部自然柔和陰影、邊緣反光與高光、忠實呈現材質本身的紋理（霧面／金屬／矽膠等），避免過度乾淨的塑膠感與 AI 感，像 MOMO 官方商品攝影。畫面上其他所有元素（標題、標籤、小圖、ICON）都必須明顯小於商品、不可搶主體。只呈現「單一顏色」的商品——多色款只放最具代表性的一色，不要在主圖並排多色，其他顏色交給選項圖。
・使用特寫：左側放 1 張「使用中的近距特寫」小圖，捕捉最能體現商品核心賣點的那個「關鍵動作瞬間」（不是普通擺放或籠統使用）；只聚焦手部與商品的互動，不要遠景、不要整個廚房或大場景，明顯小於主商品。
...
・賣點區：只保留 1 個最重要賣點「${v.SP[0].title}」，用「大而具體、一眼認得出功能的 ICON」＋≤5 字說明呈現；ICON 要清楚醒目、一眼看得到（比主標小，但不要小到看不清），白底圓角框。
・促購標籤：短、動詞開頭、有衝擊力、可帶「✔」勾號（文字：${v.SLOGAN}）；精緻設計、明顯小於商品、不搶主體、不遮擋商品，${v.ACCENT} 底、白色粗字，放在不干擾主體的留白處。
