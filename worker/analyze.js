...
- main_title_options：3 個主標候選。雙行結構，用「｜」分隔上下行。**長度硬規則（泡泡字友善）：上、下行各 ≤5 字，雙行合計 ≤10 字；太長的候選一律不要產。** 短才撐得起藝術字張力（實測「解壓捏捏製冰盒」比「捏捏樂製冰格療癒解壓」氣勢強）。含品類詞、有記憶點、蝦皮爆款口吻。用「台灣消費者實際會搜尋、會念出來的口語詞」，不要生硬書面詞或自創術語（例：「解壓製冰格」太書面，改「捏捏樂製冰格」更貼近搜尋習慣）。例：「小貓杯｜萌翻辦公室」。
- sub_title：一句 ≤12 字的利益點。例：輕鬆洗到底、秒收納不佔位。
- hero_slogan：≤6 字的行動標語，越短越有衝擊力，用動詞或利益點開頭，可用「✔」開頭強調。例：✔ 秒脫模、不用敲不用打、超省空間！
...
## 任務七：關鍵動作（主圖最該有的那個畫面，要有「落點」）
主圖要爆，關鍵常在「最能體現賣點的那個動作瞬間」，而不是靜態擺放。輸出 copy.key_action_options：3 個候選，各 ≤12 字，描述「最能體現這個商品核心賣點的動作瞬間」。**優先描述「動作＋去哪（落點／接收物）」**——有落點畫面才有故事、動線才收得住（實測「冰塊掉進玻璃杯」比只寫「冰塊掉出」強太多）。例：製冰盒 → ["手擠壓、冰塊掉進玻璃杯", "倒扣脫模、冰塊落進杯裡", "捏一捏的療癒感"]；保溫杯 → ["單手一鍵彈蓋、熱氣冒出", "倒滿熱水、蒸氣上升", "手握展示、不燙手"]。要具體、有畫面感、扣住主賣點。
